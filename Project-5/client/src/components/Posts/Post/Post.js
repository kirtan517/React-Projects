	...
	const Likes = () => {
		if (post.likes.length > 0 && isLogedIn) {
			return post.likes.find(
				(like) => like === ( user.googleId || (user.result !== undefined && user.result._id))
			) ? (
				<>
		...
		return (
			<>
				{" "}
				<ThumbUpAltOutlinedIcon fontSize="small">&nbsp;Like</ThumbUpAltOutlinedIcon>
			</>
		);
	};
	...
	return (
		<Card className={classes.card}>
			...
			<CardActions className={classes.cardActions}>
				<Button size="small" disabled = {user === null} color="primary" onClick={() => likeHandler()}>
					<Likes></Likes>
				</Button>
